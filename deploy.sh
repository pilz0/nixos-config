if [[ "$1" == "apply-local" ]]; then
    colmena $1 --impure --sudo --nix-option "experimental-features" "pipe-operator flakes nix-command" --verbose
elif [[ "$1" == "mac" ]]; then
    ...
    colmena $1 --impure --nix-option "experimental-features" "pipe-operator flakes nix-command" --on "*$2*" --verbose
fi
